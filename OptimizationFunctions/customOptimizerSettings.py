...
class MyTakeStepRDOF:
    def __init__(self, halfNumSources,stepsize=0.45):
        self.stepsize=stepsize
        self.halfNumSources=halfNumSources
        self.rng=np.random.default_rng()
    def __call__(self, x):
        s=self.stepsize
        x[1:self.halfNumSources]+=self.rng.uniform(-s,s,x[0:self.halfNumSources-1].shape)
        x[self.halfNumSources+2:2*self.halfNumSources]+=self.rng.uniform(-s,s,x[0:self.halfNumSources-2].shape)
        #x[2*self.numSources:]=x[2*self.numSources:]*self.rng.uniform(0,2,x[2*self.numSources:].shape) #multiply by random number between 0 and 2 - same as max step size of itself
        x[2*self.halfNumSources+1:]+=self.rng.uniform(-1,1,x[2*self.halfNumSources+1:].shape)
        return x
